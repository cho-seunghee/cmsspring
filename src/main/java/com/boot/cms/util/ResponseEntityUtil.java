...
public class ResponseEntityUtil {
    ...
    //정상적으로 데이터만 출력
    public <T> ResponseEntity<ApiResponse<T>> okBodyEntity(T data) {
        return ResponseEntity.ok(new ApiResponse<>(true, data, "00", ""));
    }

    //정상적이지만 에러코드와 에러메시지를 출력
    public <T> ResponseEntity<ApiResponse<T>> okBodyEntity(T data, String cd, String msg) {
        return ResponseEntity.ok(new ApiResponse<>(true, data, cd, msg));
    }

    //네트워크 오류 등 비정상적일때 출력
    public <T> ResponseEntity<ApiResponse<T>> errBodyEntity(String msg) {
        return ResponseEntity.badRequest()
                .body(new ApiResponse<>(false, null, "01", msg));
    }

    //네트워크 오류 등 비정상적일때 출력
    public <T> ResponseEntity<ApiResponse<T>> errBodyEntity(String msg, int statusCode) {
        return ResponseEntity.status(statusCode)
                .body(new ApiResponse<>(false, null, "01", msg));
    }

    // 여러 파라미터 키를 검증하고 리스트 조회를 처리하는 공통 메서드
    public <T> ResponseEntity<ApiResponse<List<T>>> handleListQuery(
        ...
        try {
            ...
            return errBodyEntity("조회 중 오류 발생: " + e.getMessage());
        }
